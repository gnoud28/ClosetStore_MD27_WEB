...
import { getAllCommetAction, DeleteCommentAction } from "../../redux/action/CommentAction";

export default function Comment() {
    const dispatch = useDispatch();
    const { arrComment } = useSelector((root) => root.CommentReducer);
    ...
    useEffect(() => {
        const action1 = getAllCommetAction();
        dispatch(action1);
    }, []);

    useEffect(() => {
        console.log("Dữ liệu arrComment khi nhận được từ Redux:", arrComment);
        setProducts(arrComment);
    }, [arrComment]);
    ...
    const deleteProduct = async () => {
        const action = DeleteCommentAction(product.comment_id);
        dispatch(action);
        setDeleteProductDialog(false);
        setProduct(emptyProduct);
    ...
    const header = (
        <div className="flex flex-wrap gap-2 align-items-center justify-content-between">
            <h4 className="m-0 mb-4">Quản lý đánh giá</h4>
            ...
            <div>
                ...
                <div className="card">
                    ...
                    <DataTable
                        ...
                        currentPageReportTemplate="Đang hiển thị {first} đến {last} trong tổng số {totalRecords} đánh giá"
